...
if (!window.__ADMIN_JS__) {
    ...
    (function() {
        ...
        async function waitForFirebase() {
            let retries = 20; // 6 segundos aprox
            while (retries > 0) {
                if (
                    window.firebase &&
        ...
        class AdminPanel {
                ...
                try {
                    ...
                    auth.onAuthStateChanged(async(user) => {
                        ...
                        try {
                            ...
                            const role = data?.role || "member"; // ✅ limpio y correcto

                            const roleLabel = document.getElementById("adminRole");
                            if (roleLabel) roleLabel.textContent = role.toUpperCase();

                            const path = location.pathname;
                            if (path.includes("dashboard.html")) this.loadDashboard();
                            else if (path.includes("users.html")) this.loadUsers();
                            else if (path.includes("sections.html")) this.loadSections();
                            else if (path.includes("documents.html")) this.loadDocuments();
                            else if (path.includes("news.html")) this.loadNews();
                        } catch (err) {
            ...
            async loadUsers() {
              ...
              <tr>
                ...
                reloadBtn?.addEventListener("click", () => this.loadUsers());
            }
